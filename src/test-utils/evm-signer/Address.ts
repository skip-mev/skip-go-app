...
export class Address {
   ...
   */
  static fromBech32(bech: string, prefix: string = "cosmos"): Address {
    try {
   ...
   * @throws {Error} if bech is not a valid bech32-encoded address
   */
  static fromHex(hex: string, prefix: string = "cosmos"): Address {
   ...
   * @returns {string}
   */
  toBech32(prefix: string = "cosmos"): string {
    const address = this.toHex();
    const addressHex = address.startsWith("0x") ? address : `0x${address}`;
    const addressBuffer =
      EthereumUtilsAddress.fromString(addressHex).toBuffer();
   ...
   * */
  toAccountAddress(prefix: string = "cosmos"): string {
    return this.toBech32(prefix);
  }
   ...
   * */
  toValidatorAddress(prefix: string = "cosmosvaloper"): string {
    return this.toBech32(prefix);
  }
   ...
   * */
  toConsensusAddress(prefix: string = "cosmosvalcons"): string {
    return this.toBech32(prefix);
  }
